<div id="CN_CompactNavigator">
	<div id="CN_NavigatorDetails">
		...
		<ul id="CN_SwitchView">
			...
			<% if DisplayMode == Stage %>
				<li><a style="left : -1px;" href="$Link?stage=Live"><% _t('CMSNavigator.VIEWPUBLISHEDSITE','View Published Site') %></a></li>
			
			<% else_if DisplayMode == Live %>
				<li><a style="left : -1px;" href="$Link?stage=Stage"><% _t('CMSNavigator.VIEWDRAFTSITE','View Draft Site') %></a></li>			
			...
			<li><a style="left : -1px;" href="$Link?flush=all"><% _t('CMSNavigator.FLUSHCACHE','Flush cache') %></a></li>
			<li><a style="left : -1px;" href="dev/build?returnURL=$BaseHref/$Link&amp;flush=all"><% _t('CMSNavigator.REBUILDSITE','Rebuild Site') %></a></li>		
			<li><a style="left : -1px;" href="AddContent&amp;flush=all"><% _t('CMSNavigator.ADDCONTENTUSINGTXTFILE','Add Content using txt file') %></a></li>		
		</ul>
